...
class Name(Expr):
    ...
    def eval(self, env):
        ...
        if self.var_name not in env:
            ...
            return env[self.var_name]             # Returns dictionary stored value.
...
class CallExpr(Expr):
    ...
    def eval(self, env):
        ...
        """
        "*** YOUR CODE HERE ***"
        operator_val = self.operator.eval(env)                                # Evaluates operator (returns dictionary stored value).
        operands_val = [element.eval(env) for element in self.operands]       # Evaluates operands.
        return operator_val.apply(operands_val)                               # Applies the value of the operator, a function, to the value(s) of the operand(s).
