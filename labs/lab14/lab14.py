...
def prune_min(t):
    ...
    while len(t.branches) > 1:
        largest_branch = max(t.branches, key = lambda b: b.label)    # Recall for the max function, the key is a function that computes a value to each element in the list that is different from its representational value. 
                                                                     # Whichever element has the largest value as computed by this function is the max of the list.
        t.branches.remove(largest_branch)                            
