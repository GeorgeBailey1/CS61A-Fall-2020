...
class Player:
    def __init__(self, deck, name):
        ...
        >>> test_deck = Deck([test_card.copy() for _ in range(6)])         #brings up the deck class so our deck name is test_deck
        >>> test_player = Player(test_deck, 'tester')                      #test deck is a deck class object, hence we can use draw() on it
        >>> len(test_deck.cards)
        1
