...
class Place:
    ...
    def __init__(self, name, exit=None):
        ...
        """
        self.name = name
        self.exit = exit      # A square to the left of the ant place
        self.bees = []        # A list of Bees
        self.ant = None       # An Ant
        self.entrance = None # A square to the right of the ant place
        # Phase 1: Add an entrance to the exit
        # BEGIN Problem 2
        "*** YOUR CODE HERE ***"
        if self.exit != None:       # Keeps track of entrances using exits.
            exit.entrance = self
        # END Problem 2
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        while current_place != beehive:
            if rANTdom_else_none(current_place.bees) != None and self.min_range <= displacement <= self.max_range:
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        if len(self.place.bees) != 0:
            for bee in self.place.bees[:]:          # This slice notation '[:]' creates a copy of the original list, which is necessary as if we just used the original list,
                                                    # when items are removed the for index is messed up and elements are skipped from the original list.
                
                Insect.reduce_armor(bee,amount)     # We have to use this instead of 'bee.reduce_armor' because the function 
                                                    # we are in is called 'reduce_armor' as well as the one defined at the start.
...
class HungryAnt(Ant):
    ...
    def __init__(self, armor=1):
        ...
        "*** YOUR CODE HERE ***"
        self.digesting = 0
        Ant.__init__(self, armor)            # Gives armor value.
        # END Problem 6

    def eat_bee(self, bee):
        # BEGIN Problem 6
        "*** YOUR CODE HERE ***"
        bee.reduce_armor(bee.armor)                                        # Can't do 'bee.armor = 0' as the value is reset everytime we call the function.
    ...
    def action(self, gamestate):
        # BEGIN Problem 6
        "*** YOUR CODE HERE ***"
        victim = rANTdom_else_none(self.place.bees)
        if victim != None:  
...
# END Problem EC
    ...
    def __init__(self, armor=1):
        ...
        if QueenAnt.counter == 0:
            self.first_queen = True          # Self attribute makes it different for different queens.
        else:
            self.first_queen = False
        ScubaThrower.__init__(self, armor)      # Only works as it is a direct sublcass of ScubaThrower, 
        self.buffed_ants = []                   # 'Ant.__init__(self, armor)' wouldn't work as parent constructor inheritance only works with direct subclasses,
                                                # could do 'super().__init__(armor)' instead to do the same thing, inheriting from a parent constructor.
        # END Problem EC
    ...
    def action(self, gamestate):
        ...
        if self.first_queen == False:
            ...
            while current_place != None:
                if current_place.ant != None and current_place.ant not in self.buffed_ants:
    ...
    def reduce_armor(self, amount):
        ...
        # BEGIN Problem EC
        "*** YOUR CODE HERE ***"
        ScubaThrower.reduce_armor(self,amount)                       # Scoober thrower is the first parent class with this function.
        if self.armor <= 0 and self.first_queen == True:
            bees_win()
