...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    """
    # Evaluate atoms
    if scheme_symbolp(expr):          # Defined in 'scheme_builtins', checks if 'expr' is a string.
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        if isinstance(rest, Pair):             # If rest is a pair we recursively send it back.
            
            operands = rest.map(new_eval)      # 'scheme_evals' each individual operand in turn, using the environment to check their value,
                                               # looking up names in the current environment, returning self-evaluating expressions (like numbers) and evaluating special forms.
...
def eval_all(expressions, env):
    ...
    if expressions is nil:
        ...
        return scheme_eval(expressions.first, env)           # Only want last value.
    else:
...
class Frame(object):
    ...
    def lookup(self, symbol):
        ...
        if symbol in self.bindings:
            ...
            return Frame.lookup(self.parent, symbol)            # Cannot do 'return self.parent.bindings[symbol]' because the parent of a parent 
                                                                # could be containing the symbol hence we do it recursively.
        # END PROBLEM 2
...
class BuiltinProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        while current_element is not nil:
            python_args.append(current_element.first)
            current_element = current_element.rest
...
class LambdaProcedure(Procedure):
    ...
    def make_call_frame(self, args, env):
        ...
        # BEGIN PROBLEM 11
        "*** YOUR CODE HERE ***"
        new_frame = Frame(self.env).make_child_frame(self.formals, args)           # Makes child frame binding the formal parameters to argument values.
        return new_frame
        # END PROBLEM 11
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 5
        "*** YOUR CODE HERE ***"
        Frame.define(env,target, scheme_eval(expressions.rest.first,env))     # Self, symbol, target respectively.
        return target
        ...
        # BEGIN PROBLEM 9
        "*** YOUR CODE HERE ***"
        env.define(target.first, do_lambda_form(Pair(target.rest, expressions.rest), env))  # Sets the environment to have 'target.first' dictionarily defined to be the expression to the right of it, 
                                                                                            # 'target.rest' being the formals or inputs of the expression and 'expressions.rest' 
                                                                                            # being the body of the expression.
        return target.first                                                                          
        # END PROBLEM 9
    else:
...
def do_quote_form(expressions, env):                                      # Quote is a syntax which expands to a quote expression. That is to say, 'X means (quote X), whatever X is
                                                                          # quote is an operator whose value is the argument syntax itself. For instance, the value of '(quote (+ 2 2))'
                                                                          # is the list '(+ 2 2)' itself, rather than the value '4'. Likewise, (quote a) yields the symbol a rather 
                                                                          # than the value of the expression a.
...
def do_and_form(expressions, env):
    ...
    while expressions.rest != nil:
        if is_false_primitive(scheme_eval(expressions.first, env)):             # 'is_false_primitive()' returns 'True' if 'False'.
            return False
        else:
...
def do_or_form(expressions, env):
    ...
    while expressions is not nil:
        ...
        if is_true_primitive(value):
            return value
        expressions = expressions.rest
    return is_false_primitive(scheme_eval(expressions, env, True))              # Returns 'True' if 'False'.
    ...
    while expressions is not nil:
        ...
        if is_true_primitive(test):
            ...
            if clause.rest is nil:
                ...
                return eval_all(clause.rest, env)             # Evaluate each expression in the Scheme list EXPRESSIONS in the environment ENV and return the value of the last.
            # END PROBLEM 13
        expressions = expressions.rest
...
def make_let_frame(bindings, env):
    ...
    # BEGIN PROBLEM 14
    "*** YOUR CODE HERE ***"
    while bindings != nil:                                                  # 'while binding != nil', could also do just 'while bindings:' since nil is by definition 'None' 
                                                                            # which is evaluated as False in our loops.
        
        validate_form(bindings.first, 2, 2)                                 # Validates our binding is of length 2, min 2 max 2 length.
        names = Pair(bindings.first.first, names)                           # 'bindings.first.first' gives our symbol for our first expression.
        values = Pair(scheme_eval(bindings.first.rest.first, env), values)  # 'bindings.first.rest.first' gives our value for our corresponding symbol above.
