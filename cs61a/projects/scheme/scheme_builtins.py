...
def scheme_stringp(x):
    return isinstance(x, str) and x.startswith('"')                    #checks if x has ' at the start

@builtin("symbol?")
def scheme_symbolp(x):
    return isinstance(x, str) and not scheme_stringp(x)                #only true if x is a str and doesn't begin with '
